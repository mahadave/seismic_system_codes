...
void adc_Enable(void) 
{
    ...
    CyDelayUs(3);
        ...
        adc_RESET_CR3_REG = adc_EN_PRESA;
    #elif (CY_PSOC5_ES1 || CY_PSOC3_ES2)
        adc_RESET_CR1_REG &= ~adc_DIS_PRES1;
        adc_RESET_CR3_REG &= ~adc_DIS_PRES2;
    #endif /* CY_PSOC5_ES1 || CY_PSOC3_ES2 */
...
int32 adc_CountsTo_uVolts( int32 adcCounts) 
{
    ...
    if(adc_CFG1_RESOLUTION < 12)
    {
        A = 1000000;
        B = 1;
    }
    else if(adc_CFG1_RESOLUTION < 15)
    {
        A = 100000;
        B = 10;
    }
    else if(adc_CFG1_RESOLUTION < 18)
    {
        A = 10000;
        B = 100;
    }
...
void adc_InitConfig(uint8 config)
{
    ...
    if (config == 1u)
    {
        ...
        adc_DSM_REF0_REG    = adc_CFG1_DSM_REF0;    
        ...
        */
        ...
        if(config == 2u)
        {
            ...
            adc_DSM_REF0_REG    = adc_CFG2_DSM_REF0;    
        ...
        if(config == 3u)
        {
            ...
            adc_DSM_REF0_REG    = adc_CFG3_DSM_REF0;    
        ...
        if (config == 4u)
        {
            ...
            adc_DSM_REF0_REG    = adc_CFG4_DSM_REF0;    
    ...
    {   
       /* Set the flag to ensure start() API dont override the config
        ...
        if(config == 1u)
        {
            ...
            clockDivider = adc_RoundValue((uint32)BCLK__BUS_CLK__HZ,
        ...
        #if(adc_DEFAULT_INTERNAL_CLK)
            adc_theACLK_SetDividerRegister(clockDivider, 1);
        #endif /* adc_DEFAULT_INTERNAL_CLK */
